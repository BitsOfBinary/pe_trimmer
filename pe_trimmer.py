...
class ParsedPEHeader:
    ...
    def calculate_checksum(self):
        self.pe_checksum = struct.unpack("I", self.pe_data[self.checksum_offset: self.checksum_offset + 4])[0]
    ...
    def calculate_optional_header_size(self):
        ...
        self.optional_header_size = \
            struct.unpack("H", self.pe_data[size_of_optional_header_offset: size_of_optional_header_offset + 2])[0]

    def calculate_section_table_offset(self):
        self.section_table_offset = self.optional_header_offset + self.optional_header_size
    ...
    def calculate_final_section_table_entry_offset(self):
        ...
        self.final_section_table_entry_offset = self.section_table_offset + (self.number_of_sections - 1) * 40

    def calculate_final_section_offset(self):
        self.final_section_offset = struct.unpack("I", self.pe_data[self.final_section_table_entry_offset + 20:
                                                                    self.final_section_table_entry_offset + 24])[0]

    def calculate_final_section_raw_size(self):
        self.final_section_raw_size = struct.unpack("I", self.pe_data[self.final_section_table_entry_offset + 16:
                                                                      self.final_section_table_entry_offset + 20])[0]

    def calculate_end_of_final_section_offset(self):
        self.end_of_final_section_offset = self.final_section_offset + self.final_section_raw_size
